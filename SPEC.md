...
| K3 | **The Game ID is stable.** Generated once, kept in `/data/gameid`, announced on Discord after every launch in relay mode (direct-connect players get the address and the join password in the START message instead). Players never need a new one after a restart or an update. |
...
| K6 | **A direct-connect server always announces its password.** Direct connections always take one: `GAME_PASSWORD`, or one the server generates when that is empty or invalid. `game_join_password` reports whichever `GameInfo.txt` names, so START never omits it or states the wrong one. Relay mode takes no password and reports none. |
| K7 | **A direct-connect server announces an address its clients can use.** The game's "join via IP" field does not resolve host names — it queries `www.` + whatever was typed, which resolves to nothing — so `game_server_address` resolves `SERVER_ADDRESS` to its IPv4 at every launch and START carries that. The configuration keeps the host name, so the tunnel provider still owns the address and the announced one cannot drift from the record. |
...
| `game_ready` | `Started session with info` in the log **and** a non-empty `GameInfo.txt` — that file is what players join with, so nothing is announced before it exists. In relay mode, on the first ready check per launch, sends the `GAMEID` notification with the Game ID. |
| `game_server_address` | Direct connect: `SERVER_ADDRESS` with its host resolved to IPv4 (already-numeric addresses pass through; an unresolvable one fails so the toolkit warns and announces the configured value). Relay mode: `2`. |
| `game_join_password` | Direct connect: the `Password:` line of `GameInfo.txt` (fails when it is missing, so the toolkit warns rather than announcing no password). Relay mode: prints nothing. |
...
- `tests/*.bats` (inside the built image): parameter compilation, Game ID generation, validation
  and persistence, `GameInfo.txt` parsing, the join password and resolved address reported per
  mode, readiness waiting
  for `GameInfo.txt`, the JOIN/LEAVE parser against fixture lines, build-id and update detection
  from manifest fixtures, `game_install`'s SteamCMD invocation.
- `tests/smoke.sh` (real SteamCMD install): session up → Game ID announced and persisted → health →
  ...
  world's newest file mtime advanced on `SIGTERM`** → direct connect with no password (START
  announces the one the server generated, as named in `GameInfo.txt`) → direct connect with
  `GAME_PASSWORD` (the server enforces it and START announces it), with START carrying the
  address resolved from `SERVER_ADDRESS` rather than the host name itself. Result recorded below after the first run.
